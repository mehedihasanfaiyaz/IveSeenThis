...
new #[Title('Edit issue')] class extends Component {
    ...
    public function mount(Issue $issue): void
    {
        ...
        $this->issue = $issue->load(['attachments', 'attempts.attachments', 'solutions.attachments']);
    ...
    <div class="workspace-frame">
        ...
        <div class="editor-grid">
            <form wire:submit="save" class="surface space-y-10 p-6 sm:p-8">
                ...
                <section id="solution" class="editor-section space-y-5"><div><p class="eyebrow">{{ __('05 / Solution') }}</p><flux:heading size="lg" class="mt-1">{{ __('Record what worked') }}</flux:heading></div><flux:input wire:model="solution_title" :label="__('Solution title')" /><flux:textarea wire:model="solution" :label="__('Solution')" rows="6" /><div class="rounded-lg border border-dashed border-emerald-500/50 p-4"><label class="text-sm font-medium">{{ __('Add a solution screenshot') }}</label>@if ($issue->solutions->first()?->attachments->isNotEmpty())<div class="mt-3 grid gap-3 sm:grid-cols-2">@foreach ($issue->solutions->first()->attachments as $attachment)<a href="{{ $attachment->url() }}" target="_blank"><img src="{{ $attachment->url() }}" alt="{{ $attachment->original_name }}" class="max-h-48 w-full rounded-lg border object-contain" /></a>@endforeach</div><flux:text class="mt-3">{{ __('Existing image. Add another to keep both.') }}</flux:text>@endif<input wire:model="solutionImage" type="file" accept="image/*" class="mt-3 block w-full text-sm" />@error('solutionImage')<flux:text color="red">{{ $message }}</flux:text>@enderror</div></section>
