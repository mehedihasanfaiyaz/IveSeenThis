...
<section class="w-full">
    <div class="mx-auto flex w-full max-w-5xl flex-col gap-8">
        ...
        <div class="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
            <div class="space-y-6">
                <section><flux:heading size="lg">{{ __('Problem') }}</flux:heading><flux:text class="mt-3 whitespace-pre-line">{{ $issue->problem }}</flux:text>@if ($issue->attachments->isNotEmpty())<div class="mt-4 grid gap-3 sm:grid-cols-2">@foreach ($issue->attachments as $attachment)<a href="{{ $attachment->url() }}" target="_blank"><img src="{{ $attachment->url() }}" alt="{{ $attachment->original_name }}" class="max-h-72 w-full rounded-lg border object-contain" /></a>@endforeach</div>@endif</section>
                ...
                <section><flux:heading size="lg">{{ __('What I tried') }}</flux:heading>@if ($issue->attempts->isNotEmpty())<div class="mt-3 space-y-5">@foreach ($issue->attempts as $attempt)<div><div class="flex gap-3"><flux:badge :color="$attempt->result === 'worked' ? 'green' : ($attempt->result === 'wrong' ? 'amber' : 'red')">{{ ucfirst($attempt->result) }}</flux:badge><flux:text>{{ $attempt->description }}</flux:text></div>@if ($attempt->attachments->isNotEmpty())<div class="mt-3 grid gap-3 sm:grid-cols-2">@foreach ($attempt->attachments as $attachment)<a href="{{ $attachment->url() }}" target="_blank"><img src="{{ $attachment->url() }}" alt="{{ $attachment->original_name }}" class="max-h-56 w-full rounded-lg border object-contain" /></a>@endforeach</div>@endif</div>@endforeach</div>@else<flux:text class="mt-3">{{ __('No attempts recorded yet.') }}</flux:text>@endif
